...
namespace winrt::winsdkfb::implementation
{
	DependencyProperty FacebookProfilePictureControl::_userIdProperty = DependencyProperty::Register(
		...
		TypeName(xaml_typename<winsdkfb::FacebookProfilePictureControl>()),
	...
	winsdkfb::CroppingType FacebookProfilePictureControl::CropMode()
	{
		return _cropMode;
	}

	void FacebookProfilePictureControl::CropMode(winsdkfb::CroppingType const& value)
	{
		if (_cropMode != value) {
			_cropMode = value;
			Update();
	...
	{
		auto instance = d.as<winsdkfb::FacebookProfilePictureControl>();
	...
	IAsyncOperation<winsdkfb::FacebookResult> FacebookProfilePictureControl::GetProfilePictureInfoAsync(int width, int height) {
		PropertySet parameters;
		wchar_t whBuffer[INT_STRING_LENGTH];
		winsdkfb::FacebookResult result;
