...
namespace winrt::winsdkfb::implementation
{
	...
	winsdkfb::FBError FBError::FromUri(Uri const& ResponseUri)
	{
		auto result = winrt::make_self<FBError>();
		...
		return result.as<winrt::winsdkfb::FBError>();
	}

	winsdkfb::FBError FBError::FromJson(hstring const& JsonText)
	{
		com_ptr<FBError> result{ nullptr };
		...
		if (JsonValue::TryParse(JsonText, val))
		{
			...
			result = winrt::make_self<FBError>();
			int found = 0;
