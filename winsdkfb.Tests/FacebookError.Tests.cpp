...
#include "../winsdkfb/FacebookError.h"
...
namespace winsdkfb_Tests
{
    ...
    public:
        TEST_METHOD(TestCreateInstance)
        {
			...
			// act
			auto fbError = ref new winsdkfb::FacebookError(0, type, message);
		...
		TEST_METHOD(TestCreationFromJson)
		{
			...
			// act
			auto fbError = winsdkfb::FacebookError::FromJson(json);
		...
		TEST_METHOD(TestCreationFromUri)
		{
			...
			// act
			auto fbError = winsdkfb::FacebookError::FromUri(uri);
